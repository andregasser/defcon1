...
export const en: Dict = {
  ...
  topbar: {
    ...
    today: 'Today',
    todayTitle: 'Today workspace: daily plan, focus, urgent and blocked tasks (t)',
    defconLabel: 'Defcon',
    hideDone: 'Slim Done',
  ...
  today: {
    heading: 'Your day. Your focus.',
    eyebrow: 'Daily workspace',
    intro: 'Plan deliberately. Start one task. See it through.',
    date: (date: string) => `Today · ${date}`,
    projects: 'Choose projects',
    allProjects: 'All projects',
    projectCount: (count: number) => `${count} projects selected`,
    all: 'Show everything',
    dueCount: (count: number) => `${count} due today`,
    overdueCount: (count: number) => `${count} overdue`,
    completedCount: (count: number) => `${count} completed today`,
    planTitle: 'Your daily plan',
    progress: (done: number, total: number) => `${done} of ${total} planned tasks completed`,
    noPlan: 'Nothing planned yet. Choose tasks for your day.',
    planHint: 'Planning does not change due dates. Unfinished tasks from previous days can be planned again.',
    chooseTasks: 'Choose tasks',
    hideChooser: 'Close task picker',
    available: 'Other tasks',
    availableEmpty: 'No other tasks in the current project focus.',
    planned: 'Work on today',
    plannedHint: 'Your deliberate selection for today.',
    plannedEmpty: 'Tasks you plan for today will appear here.',
    attention: 'Needs attention',
    attentionHint: 'Due, urgent, in progress or ready for follow-up.',
    waiting: 'Waiting for …',
    waitingHint: 'Resolve blockers and follow up at the right time.',
    completed: 'Completed today',
    focus: 'In focus now',
    focusEmpty: 'Choose deliberately what to work on next.',
    focusHint: 'One task at the center. Urgent work stays visible below.',
    focusTask: 'Focus on this task',
    clearFocus: 'Release focus',
    next: 'Possible next task',
    start: 'Start',
    resume: 'Resume work',
    done: 'Complete',
    reopen: 'Reopen',
    ...
    plan: 'Plan for today',
    unplan: 'Remove from daily plan',
    priority: 'Change priority',
    checklist: 'Checklist',
    checklistProgress: (done: number, total: number) => `${done} of ${total} steps`,
    noReason: 'No reason yet — add one in the details.',
    dueToday: 'Due today',
    overdueDays: (days: number) => `${days} days overdue`,
    dueOn: (date: string) => `Due on ${date}`,
    reviewOn: (date: string) => `Follow up on ${date}`,
    reviewReady: 'Follow-up is due',
    earlierPlan: (date: string) => `Still open from ${date}`,
    critical: 'Urgent · DEFCON 1–2',
    resetFilters: 'Reset filters',
    filtersHint: 'Search and DEFCON filters also apply here. Counters refer to the selected projects.',
    noMatches: 'No tasks match this selection.',
    undo: 'Undo',
    changed: (title: string) => `Updated: ${title}`,
    undone: 'Last change undone.',
    undoExpired: 'This task has changed since. That action can no longer be undone.',
    keyboard: 'Arrow keys move between tasks · Enter opens details · x completes · Ctrl/⌘ Z undoes',
  ...
  taskDialog: {
    ...
    reviewOn: 'Follow-up date',
    blockedReason: 'Waiting for / Blocked by',
    planningHint: 'Planning and follow-ups do not change the due date.',
    title: 'Edit task',
    titleLabel: 'Title',
  ...
  help: {
    ...
    keys: {
      ...
      today: 'Today workspace: daily plan, focus, urgent and blocked tasks',
